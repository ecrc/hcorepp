...
#pragma omp parallel default(none) shared(num_of_threads)
    ...
    DebuggingTimer::SetTimersCount(num_of_threads);
    ...
    if (!aSnapshotName.empty()) {
        ...
        for (int i = 0; i < num_of_threads; i++) {
            DebuggingTimer *timer = DebuggingTimer::GetDebuggingTimer(i);
            if (timer != nullptr) {
...
#pragma omp parallel for collapse(2) default(none) shared(aMatrixA, aMatrixB, aMatrixC, aSnapshotName, aTimer, \
        c_nt, c_mt, b_mt, aAlpha, aAOp, aBOp, aBeta, aParameters, aContext) reduction(+:flops)
        ...
        for (int j = 0; j < c_mt; j++) {
            ...
            for (int k = 0; k < b_mt; k++) {
                ...
                hcorepp::api::HCore<T>::Gemm(aAlpha, *a_tile, aAOp, *b_tile,
                                             aBOp, aBeta, *c_tile, aContext, flops, memoryHandler.GetMemoryUnit(thread_id), aParameters);
            }
        }
    }
...
#ifdef USING_TIMER
        std::vector<std::string> snapshot_names = DebuggingTimer::GetDebuggingTimer()->GetSnapshotsNames();
        std::stringstream ss;
