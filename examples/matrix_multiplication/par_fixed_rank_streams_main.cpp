...
#ifdef USE_CUDA
#define STREAMS MAX_NUM_STREAMS
#else
#define STREAMS MAX_NUM_STREAMS
#endif
...
size_t tile_matrix_multiplication(TileMatrix<T> &aMatrixA,
    ...
    std::vector<size_t> sizes(STREAMS);
    int64_t idx = 0;
    ...
    if (!aSnapshotName.empty()) {
        aTimer.StartSnapshot(aSnapshotName);
#ifdef USING_TIMER
        if (timer != nullptr) {
    ...
    if(aAllocatePool) {
        for (int i = 0; i < c_nt; i++) {
            for (int j = 0; j < c_mt; j++) {
                ...
                int64_t cxt_idx = j * aMatrixC.GetRowTileCount() + i;
                for (int k = 0; k < b_mt; k++) {
                    auto a_tile = aMatrixA.GetTile(j, k);
                    auto b_tile = aMatrixB.GetTile(k, i);
                    sizes[cxt_idx] = (std::max(sizes[cxt_idx], hcorepp::api::HCore<T>::CalculateMemoryPoolSize(*a_tile, *b_tile, *c_tile,
                                                                                                    aParameters,
                                                                                                    hcorepp::kernels::ContextManager::GetInstance().GetContext(cxt_idx))));
                }
                idx++;
            }
        }
    ...
    for (int i = 0; i < c_nt; i++) {
        for (int j = 0; j < c_mt; j++) {
            ...
            for (int k = 0; k < b_mt; k++) {
                auto a_tile = aMatrixA.GetTile(j, k);
                auto b_tile = aMatrixB.GetTile(k, i);
                auto& pool = memoryHandler.GetMemoryUnit(cxt_idx);
                pool.BufferMemSet(0, sizes[cxt_idx]);
                hcorepp::api::HCore<T>::Gemm(aAlpha, *a_tile, aAOp, *b_tile,
                                             aBOp, aBeta, *c_tile, hcorepp::kernels::ContextManager::GetInstance().GetContext(cxt_idx),
                                             flops, pool,
                                             aParameters);
            }
            idx++;
...
size_t tile_fixed_matrix_multiplication(TileMatrix<T> &aMatrixA,
    ...
    if (!aSnapshotName.empty()) {
        ...
        std::stringstream ss;
        ss << aParameters.GetAccuracy();
        std::ofstream time_file(aSnapshotName + "_" + std::to_string(aMatrixC.GetN()) + "_" + ss.str() + "_fixed" +
                               + ".time");
...
int main(int argc, char *argv[]) {
    ...
    Timer timer;
    timer.StartSnapshot("generation");
    ...
    timer.Snapshot("generation");
    {
        ...
        hcorepp::memory::Memcpy<double>(warm_a, full_a.GetData(),full_a.GetM() * full_a.GetN(), context,hcorepp::memory::MemoryTransfer::HOST_TO_DEVICE);
        hcorepp::memory::Memcpy<double>(warm_b, full_b.GetData(), full_b.GetM() * full_b.GetN(), context,hcorepp::memory::MemoryTransfer::HOST_TO_DEVICE);
        hcorepp::memory::Memcpy<double>(warm_c, full_c.GetData(), full_c.GetM() * full_c.GetN(), context,hcorepp::memory::MemoryTransfer::HOST_TO_DEVICE);
        context.Sync();
        hcorepp::kernels::HCoreKernels<double>::Gemm(blas::Layout::ColMajor, trans_a, trans_b, full_c.GetM(),full_c.GetN(),
                                                     full_a.GetN(), alpha, warm_a,full_a.GetM(), warm_b,full_b.GetM(),
                                                     beta, warm_c, full_c.GetM(), context);
    ...
    // Dense Warmup
    {
        ...
        context.Sync();
        tile_matrix_multiplication<double>(a_dense, trans_a, b_dense, trans_b, c_dense, timer, alpha,
                                   beta, "");
    }
    // Dense Flow
    {
        timer.StartSnapshot("dense_creation");
        ...
        dense_flops = tile_matrix_multiplication<double>(a_dense, trans_a, b_dense, trans_b, c_dense, timer, alpha,
                                                 beta, "dense_gemm");
        // Retrieve results back from tile format for verification.
        timer.StartSnapshot("dense_error_calc");
    ...
    for (auto &accuracy: accuracy_list) {
        ...
        // Compressed Warmup
        {
            ...
            context.Sync();
            tile_matrix_multiplication<double>(a_comp, trans_a, b_comp, trans_b, c_comp, timer, alpha,
                                       beta, "", svd_parameters);
        ...
        timer.StartSnapshot("comp_creation");
        ...
        TileMatrix<double> b_comp(full_b, row_tile_size, column_tile_size, svd_parameters, context);
        {
            ...
            // Do matrix multiplication.
            auto comp_flops = tile_matrix_multiplication<double>(a_comp, trans_a, b_comp, trans_b, c_comp, timer, alpha,
                                                         beta, "comp_gemm", svd_parameters);
            // Retrieve results back from tile format for verification.
            timer.StartSnapshot("comp_error_calc");
        ...
        timer.StartSnapshot("comp_creation");
        {
            ...
            auto comp_flops = tile_fixed_matrix_multiplication(a_comp, trans_a, b_comp, trans_b, c_comp, timer, alpha,
                                                               beta, "comp_gemm", comp_ranks, svd_parameters);
            // Retrieve results back from tile format for verification.
            timer.StartSnapshot("comp_error_calc");
            auto full_approximate_c = c_comp.ToRawMatrix(context);
            // Calculate compressed tile matrix reference error
