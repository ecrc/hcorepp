...
#            -DBUILD_SHARED_LIBS=ON
    ...
    set(ENV{LD_LIBRARY_PATH} "$ENV{LD_LIBRARY_PATH}:${${name}_installpath}/lib:${${name}_installpath}/lib64")
    set(ENV{LIBRARY_PATH} "$ENV{LIBRARY_PATH}:${${name}_installpath}/lib:${${name}_installpath}/lib64")
    set(ENV{CPATH} "$ENV{CPATH}:${${name}_installpath}/include")
    set(ENV{PKG_CONFIG_PATH} "$ENV{PKG_CONFIG_PATH}:${${name}_installpath}/lib/pkgconfig:${${name}_installpath}/lib64/pkgconfig")
    if(EXISTS ${${name}_installpath}/lib)
    	set(${capital_name}_DIR "${${name}_installpath}/lib")
	link_directories(${${name}_installpath}/lib)
    elseif(EXISTS ${${name}_installpath}/lib64)
	set(${capital_name}_DIR "${${name}_installpath}/lib64")
