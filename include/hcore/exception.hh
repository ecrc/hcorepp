...
public:
    ...
    Error(const std::string& what_arg, const char* function)
        : std::exception(),
...
// and uses printf-style format for error message condition_string is ignored,
// but differentiates this from other version.
inline void throw_if(bool condition, const char* condition_string,
...
// internal helper function; aborts if condition is true uses printf-style
// format for error message called by hcore_error_if_msg macro
inline void abort_if(bool condition, const char* function, const char* file,
...
inline void abort_if(bool condition, const char* function, const char* file,
    ...
    if (condition) {
        ...
        vsnprintf(bufffer, sizeof(bufffer), format, v);
        va_end(v);
        fprintf(stderr, "HCORE assertion failed: (%s), function %s, file %s, "
   ...
   (defined(HCORE_ERROR_ASSERT) && defined(NDEBUG))
    ...
    #define hcore_error_if(condition) \
        hcore::internal::abort_if(condition, __func__, __FILE__, __LINE__, \
                                  "%s", #condition)
    #define hcore_error_if_msg(condition, ...) \
        hcore::internal::abort_if(condition, __func__, __FILE__, __LINE__, \
                                  __VA_ARGS__)
#else
    // HCORE throws errors (default) internal macro to get string #condition;
    // throws hcore::Error if condition is true
    // Example: hcore_error_if(a < b)
    #define hcore_error_if(condition) \
        hcore::internal::throw_if(condition, #condition, __func__, __FILE__, \
                                  __LINE__)
