...
protected:
    ...
    //
    /// Empty tile.
    BaseTile() : m_(0), n_(0), data_(nullptr), ld_(0), op_(blas::Op::NoTrans),
        uplo_(blas::Uplo::General), layout_(blas::Layout::ColMajor)
        {}

    // =========================================================================
    //
    /// BaseTile that wraps existing (preallocated) memory buffer.
    /// @param[in] m
    ...
    BaseTile(int64_t m, int64_t n, T* A, int64_t ld, blas::Layout layout) : m_(m),
...
public:
    ...
    T& at(int64_t i, int64_t j)
        { return const_cast<T&>(static_cast<const BaseTile>(*this).at(i, j)); }
